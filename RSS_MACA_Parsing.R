#######################################################################################
# Script for formatting extracted MACA data to generate RSS plots
# Inputs: Baseline_all, Historical_all, and Future_all data frames generated by parsing the MACA script. Rename pr, tasmax, and tasmin columns as PrecipCustom,
#         TmaxCustom, and TminCustom
# Outputs: Formatted Baseline_all, Historical_all, and Future_all tables 
#######################################################################################
...
#####Historical_all - Daily time series of means for all GCMs
#####Baseline_all - Daily time series for all GCMs
...
Historical_all <- data.frame(aggregate(as.matrix(Baseline_all[3:ncol(Baseline_all)]) ~ Date, Baseline_all, mean))
...
Historical_all$TavgCustom <- (Historical_all$TmaxCustom + Historical_all$TminCustom)/2

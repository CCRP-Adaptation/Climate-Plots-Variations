...
DataDir = "~/RSS/Parks/HOSP"
DataFile = "HOSP_34.523_-93.061_MACA_Daily_Data_Extraction.RData"
...
###Subset future period 
Future_sub = subset(Future_all, Date$year + 1900 >= Year - Range/2 & Date$year + 1900 <= Year + Range/2)
...
Future_Means = data.frame(aggregate(cbind(Future_sub$PrecipCustom, Future_sub$TmaxCustom, Future_sub$TminCustom)
                                    ~ Future_sub$GCM, Future_sub, mean)) 
...
##Remove IPSL-CM5A-MR.rcp85 from future data
Future_Means = subset(Future_Means, GCM != "IPSL-CM5A-MR.rcp85")
Future_sub = subset(Future_sub, GCM != "IPSL-CM5A-MR.rcp85")
...
####Add column with CF classification to Future_sub/Baseline_all
CF_GCM = data.frame(GCM = Future_Means$GCM, CF = Future_Means$CF)
Future_sub = merge(Future_sub, CF_GCM[1:2], by="GCM")
...
#Add season variable for all data in Future_sub and Baseline_all
Future_sub$season=getSeason(Future_sub$Date)
...
Monthly_Tmax_Future = melt(with(Future_sub, tapply(TmaxCustom, list(Date$mon, CF), mean, na.rm=TRUE)))
Monthly_Tmax_delta = data.frame(months, CF=Monthly_Tmax_Future$Var2, Tmax_delta=Monthly_Tmax_Future$value - rep(Monthly_Tmax_Baseline, length(CFs)))
...
Monthly_Tmin_Future = melt(with(Future_sub, tapply(TminCustom, list(Date$mon, CF), mean, na.rm=TRUE)))
Monthly_Tmin_delta = data.frame(months, CF=Monthly_Tmin_Future$Var2, Tmin_delta=Monthly_Tmin_Future$value - rep(Monthly_Tmin_Baseline, length(CFs)))
...
Monthly_Precip_Future = melt(with(Future_sub, tapply(PrecipCustom, list(Date$mon, CF), mean, na.rm=TRUE)))
Monthly_Precip_delta = data.frame(months, CF=Monthly_Precip_Future$Var2, Precip_delta=Monthly_Precip_Future$value - rep(Monthly_Precip_Baseline, length(CFs)))
...
Seasonal_Precip_Future = melt(with(Future_sub, tapply(PrecipCustom, list(season, CF), mean, na.rm=TRUE)))
Seasonal_Precip_delta = data.frame(seasons, CF=Seasonal_Precip_Future$Var2, Precip_delta=Seasonal_Precip_Future$value - rep(Seasonal_Precip_Baseline, length(CFs)))
...
HistPrecip95 = quantile(Baseline_all$PrecipCustom[which(Baseline_all$PrecipCustom > 0.05)], 0.95)
...
Future_sub$TavgCustom = (Future_sub$TmaxCustom + Future_sub$TminCustom)/2
Future_sub$OverHotTemp = Future_sub$TmaxCustom > HotTemp
Future_sub$OverHighQ = Future_sub$TmaxCustom > HistTmaxHigh
Future_sub$UnderColdTemp = Future_sub$TminCustom < ColdTemp
Future_sub$UnderLowQ = Future_sub$TminCustom < HistTminLow
Future_sub$HeatConsecutive=(Future_sub$OverHotTemp)*unlist(lapply(rle(Future_sub$OverHotTemp)$lengths, seq_len))
Future_sub$ColdConsecutive=(Future_sub$UnderColdTemp)*unlist(lapply(rle(Future_sub$UnderColdTemp)$lengths, seq_len))
Future_sub$NoPrecip = Future_sub$PrecipCustom < PrecipThreshold
Future_sub$DroughtLength = (Future_sub$NoPrecip)*unlist(lapply(rle(Future_sub$NoPrecip)$lengths, seq_len))
Future_sub$OverPrecip95 = Future_sub$PrecipCustom > HistPrecip95
Future_sub$PrecipOver1 = Future_sub$PrecipCustom > 1
Future_sub$PrecipOver2 = Future_sub$PrecipCustom > 2
...
FutureOverHotTemp = MeanAnnualTotals(Future_sub, "OverHotTemp")
...
TotalOverHotTemp$CF = factor(TotalOverHotTemp$CF, levels = c("Historical", CFs))

#Cold threshold
BaseUnderColdTemp = MeanAnnualTotals(Baseline_all, "UnderColdTemp")
FutureUnderColdTemp = MeanAnnualTotals(Future_sub, "UnderColdTemp")
...
TotalUnderColdTemp$CF = factor(TotalUnderColdTemp$CF, levels = c("Historical", CFs))
...
FutureOverHighQ = MeanAnnualTotals(Future_sub, "OverHighQ")
...
TotalOverHighQ$CF = factor(TotalOverHighQ$CF, levels = c("Historical", CFs))

#Under 5th
BaseUnderLowQ = MeanAnnualTotals(Baseline_all, "UnderLowQ")
FutureUnderLowQ = MeanAnnualTotals(Future_sub, "UnderLowQ")
...
TotalUnderLowQ$CF = factor(TotalUnderLowQ$CF, levels = c("Historical", CFs))

########################################################################

#### Maximum consecutive days over hot threshold ####
BaseHeatConsecutive = MeanAnnualMax(Baseline_all, "HeatConsecutive")
FutureHeatConsecutive = MeanAnnualMax(Future_sub, "HeatConsecutive")
...
TotalHeatConsecutive$CF = factor(TotalHeatConsecutive$CF, levels = c("Historical", CFs))
...
FutureColdConsecutive = MeanAnnualMax(Future_sub, "ColdConsecutive")
...
TotalColdConsecutive$CF = factor(TotalColdConsecutive$CF, levels = c("Historical", CFs))
...
FutureDroughtLength = MeanAnnualMax(Future_sub, "DroughtLength")
...
DroughtLength$CF = factor(DroughtLength$CF, levels = c("Historical", CFs))
...
FutureSeasonalDroughtLength = MeanSeasonalMax(Future_sub, "DroughtLength")
...
SeasonalDroughtLength$CF = factor(SeasonalDroughtLength$CF, levels = c("Historical", CFs))
...
FuturePrecipMax = MeanAnnualMax(Future_sub, "PrecipCustom")
...
PrecipMax$CF = factor(PrecipMax$CF, levels = c("Historical", CFs))

##############################################

#### Annual precip over 95th percentile #####

BasePrecip95 = MeanAnnualTotals(Baseline_all, "OverPrecip95")
FuturePrecip95 = MeanAnnualTotals(Future_sub, "OverPrecip95")

BasePrecip95$CF = "Historical"
BasePrecip95$timeframe = "Historical"
Hist_Precip_95th_Days = mean(BasePrecip95$MeanAnnualTotals)

FuturePrecip95 = merge(FuturePrecip95, CF_GCM, by="GCM")
FuturePrecip95$timeframe = "Future"

TotalOverPrecip95th = rbind(BasePrecip95, FuturePrecip95)
names(TotalOverPrecip95th) = c("GCM", "TotalOverPrecip95th", "CF", "timeframe")
TotalOverPrecip95th$CF = factor(TotalOverPrecip95th$CF, levels = c("Historical", CFs))

##############################################

#### Annual precip events over 1 in. #####

BasePrecipOver1 = MeanAnnualTotals(Baseline_all, "PrecipOver1")
FuturePrecipOver1 = MeanAnnualTotals(Future_sub, "PrecipOver1")

BasePrecipOver1$CF = "Historical"
BasePrecipOver1$timeframe = "Historical"
Hist_Precip_Over1_Days = mean(BasePrecipOver1$MeanAnnualTotals)

FuturePrecipOver1 = merge(FuturePrecipOver1, CF_GCM, by="GCM")
FuturePrecipOver1$timeframe = "Future"

TotalPrecipOver1 = rbind(BasePrecipOver1, FuturePrecipOver1)
names(TotalPrecipOver1) = c("GCM", "TotalPrecipOver1", "CF", "timeframe")
TotalPrecipOver1$CF = factor(TotalPrecipOver1$CF, levels = c("Historical", CFs))

##############################################

#### Annual precip events over 2 in. #####

BasePrecipOver2 = MeanAnnualTotals(Baseline_all, "PrecipOver2")
FuturePrecipOver2 = MeanAnnualTotals(Future_sub, "PrecipOver2")

BasePrecipOver2$CF = "Historical"
BasePrecipOver2$timeframe = "Historical"
Hist_Precip_Over2_Days = mean(BasePrecipOver2$MeanAnnualTotals)

FuturePrecipOver2 = merge(FuturePrecipOver2, CF_GCM, by="GCM")
FuturePrecipOver2$timeframe = "Future"

TotalPrecipOver2 = rbind(BasePrecipOver2, FuturePrecipOver2)
names(TotalPrecipOver2) = c("GCM", "TotalPrecipOver2", "CF", "timeframe")
TotalPrecipOver2$CF = factor(TotalPrecipOver2$CF, levels = c("Historical", CFs))

######################################## END TEMP & PRECIP THRESHOLD CALCULATIONS ##############################


#### Create .xslx workbook with all data tables
dir.create(paste(OutDir, "Figs MACA", sep="/"))

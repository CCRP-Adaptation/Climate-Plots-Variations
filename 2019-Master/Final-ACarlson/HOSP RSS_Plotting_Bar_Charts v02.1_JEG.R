...
colors3 <- c("darkgray","blue", "orange")
...
TOPrecip95thH<-subset(TotalOverPrecip95th, CF == "Historical")
TOPrecip95th3<-subset(TotalOverPrecip95th, CF %in% FutureSubset & timeframe == "Future")

TOPrecip1H<-subset(TotalPrecipOver1, CF == "Historical")
TOPrecip13<-subset(TotalPrecipOver1, CF %in% FutureSubset & timeframe == "Future")

TOPrecip2H<-subset(TotalPrecipOver2, CF == "Historical")
TOPrecip23<-subset(TotalPrecipOver2, CF %in% FutureSubset & timeframe == "Future")
...
ggsave(sprintf("%s_%s_%s_Days_Under_ColdTemp.png", SiteID, Lat, Lon), width = PlotWidth, height = PlotHeight)
  ...
  #Create data frame for days/year with Tmin under historic 5th percentile value
...
TOHighQmean$HighQDays<-as.numeric(TOHighQmean$HighQDays)
      ...
      # Bar graph of days/year with Tmax over historic 95th percentile value
...
#Create data frame for days/year over historic 95th percentile value for 1-day precipitation 
TOPrecip95thmean<-ddply(TOPrecip95th3, "CF", summarise,
                        Precip95thDays=mean(TotalOverPrecip95th))
newrow <- data.frame( "CF" = character(), "Precip95thDays" = numeric(), stringsAsFactors=FALSE)
newrow[nrow(newrow) + 1, ] <- c( "Historical", Hist_Precip_95th_Days)
TOPrecip95thmean<-rbind(TOPrecip95thmean, newrow)
TOPrecip95thmean$CF<-factor(TOPrecip95thmean$CF,levels=c("Historical",Scenario1, "Central", Scenario2), ordered=is.ordered(TOPrecip95thmean$CF))
TOPrecip95thmean$Precip95thDays<-as.numeric(TOPrecip95thmean$Precip95thDays)

ggplot(TOPrecip95thmean, aes(x=CF, y=Precip95thDays, fill=CF)) +
  geom_bar(stat="identity", position="dodge") +
  PlotTheme +
  labs(title = paste0(SiteID, " - Days/Yr with Precip. > Historic 95th Percentile (", round(HistPrecip95, 1), " in.) \nin Historical (1950-2005) vs. ", Year, sep=""),
       x = "Historical & Future Climate Scenarios", y = "Days/Yr", colour = "Climate Future") +
  scale_fill_manual(name="",values = colors3)

ggsave(sprintf("%s_%s_%s_Days_Over_95th_Precip.png", SiteID, Lat, Lon), width = PlotWidth, height = PlotHeight)

#Create data frame for days/year with 1-day precip over 1 in. 
TOPrecip1mean<-ddply(TOPrecip13, "CF", summarise,
                        Precip1Days=mean(TotalPrecipOver1))
newrow <- data.frame( "CF" = character(), "Precip1Days" = numeric(), stringsAsFactors=FALSE)
newrow[nrow(newrow) + 1, ] <- c( "Historical", Hist_Precip_Over1_Days)
TOPrecip1mean<-rbind(TOPrecip1mean, newrow)
TOPrecip1mean$CF<-factor(TOPrecip1mean$CF,levels=c("Historical",Scenario1, "Central", Scenario2), ordered=is.ordered(TOPrecip1mean$CF))
TOPrecip1mean$Precip1Days<-as.numeric(TOPrecip1mean$Precip1Days)

ggplot(TOPrecip1mean, aes(x=CF, y=Precip1Days, fill=CF)) +
  geom_bar(stat="identity", position="dodge") +
  PlotTheme +
  labs(title = paste0(SiteID, " - Days/Yr with Precip. > 1 in. in Historical (1950-2005) vs. ", Year, sep=""),
       x = "Historical & Future Climate Scenarios", y = "Days/Yr", colour = "Climate Future") +
  scale_fill_manual(name="",values = colors3)

ggsave(sprintf("%s_%s_%s_Days_Over_1in_Precip.png", SiteID, Lat, Lon), width = PlotWidth, height = PlotHeight)

#Create data frame for days/year with 1-day precip over 2 in. 
TOPrecip2mean<-ddply(TOPrecip23, "CF", summarise,
                     Precip2Days=mean(TotalPrecipOver2))
newrow <- data.frame( "CF" = character(), "Precip2Days" = numeric(), stringsAsFactors=FALSE)
newrow[nrow(newrow) + 1, ] <- c( "Historical", Hist_Precip_Over2_Days)
TOPrecip2mean<-rbind(TOPrecip2mean, newrow)
TOPrecip2mean$CF<-factor(TOPrecip2mean$CF,levels=c("Historical",Scenario1, "Central", Scenario2), ordered=is.ordered(TOPrecip2mean$CF))
TOPrecip2mean$Precip2Days<-as.numeric(TOPrecip2mean$Precip2Days)

ggplot(TOPrecip2mean, aes(x=CF, y=Precip2Days, fill=CF)) +
  geom_bar(stat="identity", position="dodge") +
  PlotTheme +
  labs(title = paste0(SiteID, " - Days/Yr with Precip. > 2 in. in Historical (1950-2005) vs. ", Year, sep=""),
       x = "Historical & Future Climate Scenarios", y = "Days/Yr", colour = "Climate Future") +
  scale_fill_manual(name="",values = colors3)

ggsave(sprintf("%s_%s_%s_Days_Over_2in_Precip.png", SiteID, Lat, Lon), width = PlotWidth, height = PlotHeight)
...
Future_Tmax<-data.frame(months,with(Future_sub,tapply(TmaxCustom, list(Date$mon, CF), mean))); Future_Tmax=melt(Future_Tmax)
...
#Tmax - Scenario 2
WTmax.ab<-ddply(WTmax, "CF", summarise, mean=mean(Tmax, na.rm=TRUE))
SpTmax.ab<-ddply(SpTmax, "CF", summarise, mean=mean(Tmax, na.rm=TRUE))
SuTmax.ab<-ddply(SuTmax, "CF", summarise, mean=mean(Tmax, na.rm=TRUE))
FTmax.ab<-ddply(FTmax, "CF", summarise, mean=mean(Tmax, na.rm=TRUE))

#Create dataframe and subset Future Tmin by season
Future_Tmin<-data.frame(months,with(Future_sub,tapply(TminCustom, list(Date$mon, CF), mean))); Future_Tmin=melt(Future_Tmin)
...
#Summarize by scenario
WTmin.ab<-ddply(WTmin, "CF", summarise, mean=mean(Tmin, na.rm=TRUE))
SpTmin.ab<-ddply(SpTmin, "CF", summarise, mean=mean(Tmin, na.rm=TRUE))
SuTmin.ab<-ddply(SuTmin, "CF", summarise, mean=mean(Tmin, na.rm=TRUE))
FTmin.ab<-ddply(FTmin, "CF", summarise, mean=mean(Tmin, na.rm=TRUE))

#Create dataframe and subset Future Precip by season (=daily mean*30)
Future_Precip= data.frame(months,with(Future_sub, tapply(PrecipCustom, list(Date$mon, CF), mean, na.rm=TRUE))*30);Future_Precip=melt(Future_Precip)
...
#Summarize by scenario
WPrecip.ab<-ddply(WPrecip,"CF",summarise,sum=sum(Precip, na.rm=TRUE))
SpPrecip.ab<-ddply(SpPrecip,"CF",summarise,sum=sum(Precip, na.rm=TRUE))
SuPrecip.ab<-ddply(SuPrecip,"CF",summarise,sum=sum(Precip, na.rm=TRUE))
FPrecip.ab<-ddply(FPrecip,"CF",summarise,sum=sum(Precip, na.rm=TRUE))
